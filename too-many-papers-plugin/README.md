...
Everything lives in `_papers.json`, `_venues.json`, `_graph.json`, plus an append-only `_log.jsonl` audit trail of every mutation (written automatically by the server, not something the AI has to remember to update), and automatically-fetched PDFs under `pdfs/`. Plain JSON, version-controllable, portable.

This data directory is deliberately **not** `server/` inside the plugin's own install tree — that gets wiped and replaced on every plugin update/reinstall, and on some hosts even re-provisioned fresh every session. Where it actually lives: `${CLAUDE_PLUGIN_DATA}` if the host expands that placeholder (Claude Code does), otherwise `~/.too-many-papers`. `papers_api.py`'s `_resolve_data_dir()` treats an empty or literal unexpanded `"${CLAUDE_PLUGIN_DATA}"` value the same as "not set" and falls back accordingly — so a host that doesn't support the placeholder still gets a real, persistent directory instead of silently resetting every session. Run `graph_status` or `webui_launch` to print the exact path in use. Back up by copying that directory.
